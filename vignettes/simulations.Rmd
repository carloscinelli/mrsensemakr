...
sim <- function(nsim, n = 100, m= 90, gamma = 0.1, eta = gamma, tau = 0,  presso = FALSE,
                R1.a = FALSE, j = 1, R1.b = FALSE, k = 1, scale = TRUE){
  ...
  if(presso){
    ...
    output.presso <- mr_presso(BetaOutcome = "by", BetaExposure = "bx", SdOutcome = "byse", SdExposure = "bxse",
                               OUTLIERtest = TRUE, DISTORTIONtest = FALSE, data = res, NbDistribution = 1000,
  ...
  #MR-Mix
  output.mix <- MRMix(betahat_x = b1$bd, betahat_y = b2$by, sx = b1$bdse, sy =b2$byse,profile = FALSE)
  ...
  if(s.t != s.adj_t){
    ...
    p.bench <- pt(abs(adj_t), df = n, lower.tail = FALSE)*2
  }
  out$p.bench  <- p.bench
  out$rv       <- rv
  out$r2yz     <- r2yz
...
## test run
# system.time(test <- sim(1, n = 1e5, presso = TRUE, gamma = 0.05, tau = 0))

## now simulate as many times as you want, with different parameter values
## system.time({
#   sim.out <- grid_search(sim, 
#                          n.iter = 1000, 
#                          params = list(n = c(150000, 300000, 450000), gamma = 0.05, tau = c(0, 0.1), presso = TRUE), 
#                          parallel = "multicore", 
#                          ncpus = 32
#   )
# })
